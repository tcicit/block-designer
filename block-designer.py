...
class EditorModel(QObject):
    ...
    def insert_text_at(self, start_r, start_c, text):
        if text is None:
            return None
        text = text.replace("\r\n", "\n").replace("\r", "\n")
        if not text:
        ...
        self.expand_grid_if_needed(min_rows=start_r + len(lines), min_cols=start_c + max_width)
        ...
        self.grid_changed.emit()
        last_row = start_r + len(lines) - 1
        last_col = start_c + len(lines[-1])
        return last_row, min(last_col, self.cols - 1)
...
class CanvasWidget(QPlainTextEdit):
    ...
    def insertFromMimeData(self, mime_data):
        if not mime_data or not mime_data.hasText():
            return super().insertFromMimeData(mime_data)

        if self.model.active_tool != TOOL_TEXT:
            return False

        text = mime_data.text()
        tc = self.textCursor()
        r, c = tc.blockNumber(), tc.positionInBlock()
        end_pos = self.model.insert_text_at(r, c, text)
        self.refresh()
        if end_pos is not None:
            self._move_cursor_to(*end_pos)
        self.status_message.emit("Pasted text from clipboard.")
        return True
    ...
    def keyPressEvent(self, event):
        ...
        if mods & Qt.KeyboardModifier.ControlModifier and key == Qt.Key.Key_V:
            if tool == TOOL_TEXT:
                ...
                if self.insertFromMimeData(clipboard.mimeData()):
                    event.accept()
                    return
...
class CharPalette(QScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        self._cols = 0
        for char in CHARS:
            # Icon und Hovertext für Leerzeichen hinzufügen (Eraser) 
            display = "⌫" if char == " " else char            
            btn = QPushButton(display)
            btn.setFixedSize(QSize(34, 28))
            btn.setFont(QFont("Courier New", 22))
